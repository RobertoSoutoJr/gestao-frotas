...
export default function NovaViagemScreen() {
  ...
  const captureGpsOrigem = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permissao negada', 'Habilite a localizacao nas configuracoes.');
        return;
      }
  ...
  const validate = (): boolean => {
    ...
    if (!form.preco_produto_saca || Number(form.preco_produto_saca) <= 0)
      e.preco_produto_saca = 'Informe o preco do produto por saca';
  ...
  const handleSubmit = () => {
    ...
    mutation.mutate({
      ...
      lat_origem: gpsOrigem?.lat ?? null,
      lng_origem: gpsOrigem?.lng ?? null,
    });
  };
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      >
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          />
          <Input
            label="Preco do produto por saca (R$) *"
          ...
          >
            ...
            <View style={styles.gpsBtnText}>
              <Text style={[styles.gpsBtnTitle, gpsOrigem && { color: colors.success }]}>
                {capturingGps ? 'Capturando GPS...' : gpsOrigem ? 'GPS capturado' : 'Capturar localizacao atual'}
              </Text>
              {gpsOrigem && (
